...
const getVerseArray = function(verseRef) {
  ...
  if (!verseArr || !verseArr[0] || (verseArr && verseArr.length !== 3)) return;
...
const getDisplayVerse = function(verseRef) {
  ...
  if (!book) return;
  return `${book} ${verseArr[1]}:${verseArr[2]} (${version})`;
}
...
const getVerseRef = function(message) {
  ...
  const verseRef = result && result.components && result.components[0] && result.components[0].osis;
  if (!verseRef) return;
  return verseRef;
}
...
const getVerse = function(message, say) {
  ...
  const options = {
      url: `https://api.scripture.api.bible/v1/bibles/de4e12af7f28f599-01/passages/${verseForAPI}?content-type=text&include-notes=false&include-titles=false&include-chapter-numbers=false&include-verse-numbers=true&include-verse-spans=false&use-org-id=false`,
      method: 'GET',
      headers: {
  ...
  request(options, function (error, response, body) {
    ...
    if (!error && response.statusCode == 200) {
      ...
      const verseContent = results.data.content && results.data.content.split(']');
      const verse = verseContent && verseContent.length && verseContent[1];
      console.log(verse);

      if (verse) {
        const refLink = getReferenceLink(verseRef);
        const displayVerse = getDisplayVerse(verseRef);
        const emoji = getRandEmoji();
        try {
          say({
            blocks: [
            {
              ...
              "text": {
                "type": "mrkdwn",
                "text": `> ${getRandEmoji()} ${verse.trim()} - <${refLink}|${displayVerse}>`
              },
            }
            ]
          });
